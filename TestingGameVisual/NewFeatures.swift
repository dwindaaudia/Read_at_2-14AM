// NewFeatures.swift
// Files (Evidence Board) UI — "Read at 2:14 AM"
...
// MARK: - Encrypted file gating (matches narrative beat)

func encryptedFileDecryptAvailable(for gameManager: GameManager) -> Bool {
    if gameManager.turnCount >= 6 { return true }
    return gameManager.messages.contains { msg in
...
struct FilesEvidenceView: View {
    ...
    var body: some View {
        NavigationStack {
            ...
            .toolbarBackground(Self.bgMaroon, for: .navigationBar)
...
private struct InteractiveStoryFileCard: View {
    ...
    var body: some View {
        ...
        .fullScreenCover(isPresented: $showImageViewer) {
            if let name = imageAssetName {
                FileImageViewerSheet(assetName: name, caption: item.message.text)
            }
        }
        .sheet(isPresented: $showVoicePlayer) {
            if let fn = voiceFilename {
                FileVoicePlayerSheet(filename: fn)
            }
        }
    ...
    private func handleTap() {
        ...
        case .archive:
            if encryptedFileDecryptAvailable(for: gameManager) {
...
private struct FileVoicePlayerSheet: View {
    ...
    var body: some View {
        NavigationStack {
            ...
            .toolbarBackground(Color(red: 0.1, green: 0, blue: 0.02), for: .navigationBar)
            .toolbarBackground(.visible, for: .navigationBar)
            .toolbarColorScheme(.dark, for: .navigationBar)
        }
